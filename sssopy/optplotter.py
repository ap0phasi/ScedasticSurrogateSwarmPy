...
def plot_optimizer_results(pos_x,pos_results,xdat,ydat,surrogatesaves,centersaves,optproblem):
    plt.scatter(xdat,ydat,color="black",label = "Observed")
    
    for i, row in enumerate(pos_results):
        # Label the first line in the loop only
        label = "Modeled" if i == 0 else None
        plt.plot(xdat, row, color="grey", label=label)
...
def plot_optimizer_results_with_rec(pos_x,pos_results,xdat,ydat,surrogatesaves,centersaves,surrogate_recommendation,optproblem):
    plt.scatter(xdat,ydat,color="black",label = "Observed")
...
def plot_optimizer_results_nosurrogate(pos_x,pos_results,xdat,ydat,optproblem):
    
    for i, row in enumerate(pos_results):
        # Label the first line in the loop only
        label = "Modeled" if i == 0 else None
        plt.plot(xdat, row, color="grey", label=label, zorder = 1)
