...
def test_surrogate_on_function(fun,dim):
    ...
    # Replace cluster centers with actual values
    cluster_replace_index = np.argsort(cdist(cluster_centers,allinputs))[:,0]
    cluster_centers = allinputs[cluster_replace_index]
    
    surrogatesaves=[]
    
    # Perform surrogate modelling per cluster
    for cluster_index in range(len(cluster_centers)):
        selector = SurrogateModelSelector(fit_threshold=0.001)
        modelinputs = np.vstack([
            inputs[clustering_vector==cluster_index],
            allinputs[cluster_replace_index[cluster_index]]
            ])
        modeloutputs = np.vstack([
            outputs[clustering_vector==cluster_index],
            alloutputs[cluster_replace_index[cluster_index]]
            ])
        selector.fit_models(modelinputs, modeloutputs, cluster_centers[cluster_index])
